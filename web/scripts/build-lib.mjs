...
try {
  ...
  // Always remove the scratch dirs, even if tsc or any step above fails, so a
  // failed build never leaves stale `dist-lib-tsc*` artifacts behind.
  rmSync(tscSrc, { recursive: true, force: true });
  rmSync(tscOut, { recursive: true, force: true });
}

function applyRewrite(source, target, replacement, label) {
  if (!source.includes(target)) {
    throw new Error(
      `rewrite target not found in index.ts (${label}): ${JSON.stringify(
        target,
